...
theta_unwrapped =[] #unwrapped radians, ranges 0 to 2pi
theta_degree = [] #unwrapped degrees, ranges 0 to 360, better than radians because u can easily mulitply theta deg by steps per degree
#steps per degree is more native than steps per rev anyway and degree is easier to debug developer side
...
print(f"min(theta_array): {min(theta_array):.2f}, max(theta_array): {max(theta_array):.2f}")
print(f"min(theta_unwrapped): {min(theta_unwrapped):.2f},   max(theta_unwrapped): {max(theta_unwrapped):.2f}")
>>>>>>> 4761c2edeccab061de2906afc7e7191590913986
